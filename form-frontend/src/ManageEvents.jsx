...
const EventPreviewModal = ({ event, onClose, editable, onSave }) => {
  ...
  if (editImage) {
    ...
    imageUrl = event.main_image_path.startsWith('http') ? event.main_image_path : `/storage/${event.main_image_path.replace(/^\/+/, '')}`;
  } else if (event.main_image) {
    imageUrl = typeof event.main_image === 'string' ? event.main_image : URL.createObjectURL(event.main_image);
  }
  return (
      ...
      <div className="bg-white rounded-lg shadow-2xl p-0 w-full max-w-sm mx-auto relative overflow-hidden">
        ...
        <div className="p-4">
          ...
          <div className="mb-2">
            ...
            <span className="text-purple-700 font-semibold">{event.organizer || "(You)"}</span>
          </div>
          <div className="flex items-center text-green-700 text-sm mb-1">
            ...
            {editable ? (
              <input
                name="start_time"
                type="datetime-local"
                value={editData.start_time ? formatForDatetimeLocal(editData.start_time) : ""}
                onChange={handleChange}
                className="border rounded p-1 ml-2"
              />
            ) : (
              event.start_time ? new Date(event.start_time).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' }) : "No start time"
            )}
          </div>
          <div className="flex items-center text-green-700 text-sm mb-1">
            <svg className="w-5 h-5 mr-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
            {editable ? (
              <input
                name="end_time"
                type="datetime-local"
                value={editData.end_time ? formatForDatetimeLocal(editData.end_time) : ""}
                onChange={handleChange}
                className="border rounded p-1 ml-2"
              />
            ) : (
              event.end_time ? new Date(event.end_time).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' }) : "No end time"
            )}
          </div>
          ...
          <div className="flex items-center text-gray-600 text-xs mt-2">
            <span>Type: {editable ? (
              <select name="event_type" value={editData.event_type || ""} onChange={handleChange} className="border rounded p-1 ml-2">
            ...
            {editable ? (
              ...
              <span className="ml-4">Max Attendees: {event.max_attendees}</span>
            )}
          </div>
          <div className="flex items-center text-gray-600 text-xs mt-2">
            <span>Status: {event.is_approved ? "Active" : "Pending Approval"}</span>
          </div>
          {editable && (
            <div className="mt-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Change Image</label>
              <input type="file" name="main_image" accept="image/*" onChange={handleChange} className="mb-2" />
            </div>
          )}
          {editable && (
            <button
...
const ManageEvents = () => {
  ...
  const handleSaveEdit = async (updatedEvent) => {
    try {
      ...
      const response = await api.post(`/api/events/${updatedEvent.id}`, formData, {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {Array.isArray(events) && events.length > 0 ? (
              events.map((event) => (
                <tr key={event.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">{new Date(event.start_time).toLocaleString()}</td>
                  <td className="py-3 px-4">{event.venue}</td>
                  <td className="py-3 px-4">
                    <span className={(event.is_approved ? 'bg-green-200 text-green-800' : 'bg-yellow-100 text-yellow-700') + " py-1 px-3 rounded-full text-xs font-semibold"}>
                      {event.is_approved ? 'Active' : 'Pending Approval'}
                    </span>
                  </td>
                  <td className="py-3 px-4">
                    <button className="text-indigo-600 hover:underline mr-4" onClick={() => handlePreview(event)}>
                      Preview
                    </button>
                    <button className="text-indigo-600 hover:underline mr-4" onClick={() => handleEdit(event)}>
                      Edit
                    </button>
                    <button className="text-red-600 hover:underline" onClick={() => handleDelete(event)}>
                      Delete
                    </button>
                  </td>
                </tr>
      ...
      {previewEvent && (
        <EventPreviewModal event={previewEvent} onClose={() => setPreviewEvent(null)} editable={false} onSave={handleSaveEdit} />
      )}
      {editEvent && (
        <EventPreviewModal event={editEvent} onClose={() => setEditEvent(null)} editable={true} onSave={handleSaveEdit} />
      )}
    </div>
  );
};
